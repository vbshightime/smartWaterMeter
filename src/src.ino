...
void setup()
{
  ...
  digitalWrite(RELAY_PIN,RSTATE.isIrrigating);
...
int toggleIrrigation(char *args, char *actionId)
{
  Serial.printf("*** args : %s , actionId : %s ***\n", args, actionId);
  if (RSTATE.isIrrigating ? Serial.println("Irrigating Plants") : Serial.println("Irrigation Off"));
  RSTATE.isIrrigating = !RSTATE.isIrrigating;
  toggleRelay();
...
{
  digitalWrite(RELAY_PIN, RSTATE.isIrrigating);
  RSTATE.isIrrigating = digitalRead(RELAY_PIN);
}
...
void loop()
{
  if (!RSTATE.isPortalActive)
  {
    ...
    if (WiFi.isConnected() && !RSTATE.isBytebeamBegin)
    {
      ...
      Bytebeam.addActionHandler(toggleIrrigation, "toggle_irrigation");
...
bool publishToDeviceShadow()
{
  ...
  if (!RSTATE.isIrrigating)
  {
    deviceShadowJsonObj_1["Status"] = "Irrigating plants";
  }
  else
  {
    deviceShadowJsonObj_1["Status"] = "irrigation off";
  }
